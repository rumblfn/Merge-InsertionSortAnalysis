...
def main():
    contents = ['random_array_merge_sort.json', 'random_array_optimized.json']
    ...
    ax.set_title('Сравнение MergeSort и оптимизированного MergeSort')
    ax.set_xlabel('Длина сортируемого массива')
    ax.set_ylabel('Время, затраченное на сортировку (мс)')
    ax.grid(True)
    ax.legend()
    
    plt.savefig('../output/random-array-combined.png')
